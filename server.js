...
if (argv.location) console.log('mocknode installation directory: ', __dirname);else if (argv.export) {
  ...
  var getStubSatusCode = function getStubSatusCode(_route, _stub) {
    var stubConf = config.routes.find(function (route) {
      return route.route === _route;
    }).stubs.find(function (stub) {
      return stub.name === _stub;
    });
    return stubConf.code || 200;
  };

  var getStubHeaders = function getStubHeaders(_route, _stub) {
    var stubConf = config.routes.find(function (route) {
      return route.route === _route;
    }).stubs.find(function (stub) {
      return stub.name === _stub;
    });
    return stubConf.headers || {};
  };

  // Middleware which returns a stub
  var stubHandler = function stubHandler(_route, _stub) {
    return function (req, res, next) {
      return res.set(getStubHeaders(_route, _stub)).status(getStubSatusCode(_route, _stub)).sendFile(path.join(__dirname, 'stubs', encodeRoutePath(_route), _stub));
    };
  };
  ...
  var dynamicStubRequestHandler = function dynamicStubRequestHandler(_route, _stub) {
    return function (req, res) {
      ...
      _stub.conditions.every(function (condition) {
      ...
      // }
      // else {
      res.set(getStubHeaders(_route, returnedStub)).status(getStubSatusCode(_route, returnedStub)).sendFile(path.join(__dirname, 'stubs', encodeRoutePath(_route), returnedStub));
      // }
    };
  };
  ...
  app.get('/', function (req, res) {
    return res.redirect(301, '/mocknode');
  });
  ...
  config.routes.filter(function (configObj) {
    ...
    createStubRoute(configObj.route, configObj.stub);
  });
